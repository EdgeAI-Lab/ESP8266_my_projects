...
/*********Local variable***************/
LOCAL bool recvStart = FALSE;
LOCAL bool recvLen = FALSE;
LOCAL bool recvReally = FALSE;
LOCAL bool recvAttr = FALSE;
LOCAL bool checkStart = FALSE;
...
void timer_task(void *pvParameters)
{
	...
	for(;;)
	{
		...
		// 超过6毫秒
		if( (system_get_rtc_time() - frameStartTime) > 100000 )
		{
			//printf("%d\n",system_get_rtc_time());
			//printf("%d\n",frameStartTime);
			...
			if (xQueueReceive(xQueueUart, (void *)&e, (portTickType)portMAX_DELAY)) {
				...
				switch (e.event) {
					case UART_EVENT_RX_CHAR:
						...
						// 帧头接收完毕，开始接收数据，从这个位置到Len字段，有固定的6字节
						if(recvReally && (j++ < 6))
						{
							
							recvLen = TRUE;
							printf("data\n");
							uart_rx_buf[i++] = e.param;
							continue;
						...
						else if( recvLen )
						{
							...
							len = uart_rx_buf[i++] = e.param;
							recvAttr = TRUE;
							
							continue;
						}
						else if(recvAttr && (k++ < len) )
						{
						...
						else if(checkStart)
						{
							...
							sum = get_check_sum(uart_rx_buf,len+9);
							printf("%x\n",sum);
						...
						{
							// 获取当前时间(RTC时间)
							frameStartTime = system_get_rtc_time();
						...
						// 接收到帧头第二个字节
						if( recvStart && ( e.param == 0xA5) )
						{
							uart_rx_buf[i++] =e.param;
							printf("second\n");
							recvStart = FALSE;
							recvReally = TRUE; // 标志 可以开始接收数据
						}
 ...
 *
 */
void sendAck(uint8 *buf, uint16 len)
{    
	uint8 i; 
	...
	// 发送数据头
	uart_tx_one_char(UART0, 0x5A);   
	uart_tx_one_char(UART0, 0xA5);
	uart_tx_one_char(UART0, uart_rx_buf[ENDPOINT_ID]);
	uart_tx_one_char(UART0, uart_rx_buf[CLUSTER_ID]);
	uart_tx_one_char(UART0, uart_rx_buf[CLUSTER_ID+1]);
	uart_tx_one_char(UART0, 0x03);
	uart_tx_one_char(UART0, uart_rx_buf[ATTR_ID]);
	uart_tx_one_char(UART0, uart_rx_buf[ATTR_ID+1]);
	uart_tx_one_char(UART0, len);
	
	// 发送属性值
	for (i = 0; i < len; i++) {  
	
		uart_rx_buf[9+i] = buf[i];
		uart_tx_one_char(UART0, buf[i]);    
	}
	
	// 发送校验和
	uart_tx_one_char(UART0, get_check_sum(uart_rx_buf,len+9));
...
void sendAckNoData(void)
{
	...
	// 发送校验和（此时没有数据）
	uart_tx_one_char(UART0, get_check_sum(uart_rx_buf,9));
...
uart_init_new(void)
{
	...
	// 创建数据帧接收函数
    xTaskCreate(uart_task, (uint8 const *)"uTask", 512, NULL, tskIDLE_PRIORITY + 4, &xUartTaskHandle);
